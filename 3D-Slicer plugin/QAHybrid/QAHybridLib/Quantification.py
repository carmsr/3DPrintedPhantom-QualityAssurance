# Activity concentration, HU and ms quantification analysis for PET, CT and MR DICOM images: 3D-Printed Phantom.
...
class OptionPet(tk.Toplevel):
    def __init__(self, parent, title):
        ...
        self.entry3.insert(tk.END, "1500")
...
class OptionCT(tk.Toplevel):
    ...
    def modify_inserts(self):
        ...
        ttk.Label(self.modify_window, text="Enter new Position-Insert:").grid(
    ...
    def save_new_inserts(self):
        new_inserts = [
            (int(pos_entry.get()), name_entry.get())
            for pos_entry, name_entry in self.new_inserts_entries
            if pos_entry.get().strip() and name_entry.get().strip()
        ]
    ...
    for dir in sub_dirs:
        ...
        for sub_folder in sub_folders:
            if sub_folder == "PET":
                ...
                coord_pet = Auxiliar_functions.select_slices(
                    os.path.join(path_date, sub_folder),
                    sub_folder,
                    "Choose initial slice for quantification analysis",
                    dir,
                )[0]

                # Choose PET acquisition information
                dialog_pet = OptionPet(
                    window, f"Parameters for determining the activity concentration of the solution used for PET {dir}"
                )
                ...
                # Choose initial slice
                coord_ct = Auxiliar_functions.select_slices(
                    os.path.join(path_date, sub_folder),
                    sub_folder,
                    "Choose initial slice for quantification analysis",
                    dir,
                ...
                for mr_image_path in mr_images_path:
                    ...
                    if "T1" in type_mr:
                        coord_mrt1 = Auxiliar_functions.select_slices(
                            mr_image_path,
                        ...
                        coord_mrt2 = Auxiliar_functions.select_slices(
                            mr_image_path,
                            sub_folder + "_" + type_mr,
                            "Choose initial slice for quantification analysis",
                            dir,
        ...
        for sub_folder in sub_folders:
            if sub_folder == "CT" or sub_folder == "MR" or sub_folder == "PET":
                ...
                if sub_folder == "PET":
                    ...
                    area_pet, final_coord_pet = Auxiliar_functions.segmentation_region(
                        im, coord_pet, float(serie_pet.GetSpacing()[2]), sub_folder, "RC"
                    )

                    # CONCENTRATION analysis #
                    # Radiactive inserts segmentation
                    names_pet, centers_pet, inserts_pet = Auxiliar_functions.inserts_pet(
                        im, area_pet, coord_pet, final_coord_pet, serie_pet.GetSpacing()
                    )
                    ...
                    if radioph == "18F-FDG":
                        ...
                        activity_decay = (activity / volume) * math.exp(
                            -diff_time * ((math.log(2)) / other_semidecay_period)
                        )
                    ...
                    for i in range(len(inserts_pet)):
                        ...
                        segm_activity, diameter_insert = Auxiliar_functions.reduced_segmentation_cyl(
                            inserts_pet[i], float(serie_pet.GetSpacing()[0])
                        )

                        if not np.isnan(diameter_insert):
                            diameter_pet.append(diameter_insert)
                            # sitk.WriteImage(segm_activity, "C:\\Users\\20970341E\\Documents\\PET_Insert" + str(i) + ".nrrd")

                            # ACTIVITY quantification
                            in_pet, err_pet = Auxiliar_functions.quantification(
                                image, sitk.GetArrayFromImage(segm_activity)
                            )
                    ...
                    # Create the interest analysis area
                    area_ct, final_coord = Auxiliar_functions.segmentation_region(
                        im, coord_ct, float(serie.GetSpacing()[2]), sub_folder, inserts_type
                    )

                    # HU analysis #
                    # Select inserts
                    names_ct, centers_ct, inserts_ct = Auxiliar_functions.inserts_ct(
                        im, coord_ct, final_coord, serie.GetSpacing(), inserts_type
                    )

                    # Go through each insert
                    for i in range(len(inserts_ct)):
                        # Create a reduced analysis region inside the insert for HU quantification
                        segm_den, diameter_in_ct = Auxiliar_functions.reduced_segmentation_cyl(
                            inserts_ct[i], float(serie.GetSpacing()[0])
                        )
                    ...
                    if inserts_type == "Calibration Curve":
                        ...
                        plt.savefig(
                            os.path.join(output_date_analysis_path, f"QuantificationCT_CalibrationCurve.png"),
                            dpi=600,
                            format="png",
                            bbox_inches="tight",
                        )
                    ...
                    if len(names_ct) != 0 and len(intensity_ct) != 0:
                        ...
                        if inserts_type == "Calibration Curve":
                            dic_ct["Density (g/cm3)"] = density_list
                    ...
                    mr_images_path = [
                        os.path.join(mr_path, d) for d in os.listdir(mr_path)
                    ]  # DICOM series folder paths
                    for mr_image_path in mr_images_path:
                        intensity_mr = []
                        err_intensity_mr = []
                        diameter_mr = []
                        ...
                        # Create the interest analysis area
                        area_mr, final_coord = Auxiliar_functions.segmentation_region(
                            im, coord_mr, final_coord, float(serie_mr.GetSpacing()[2]), sub_folder, inserts_type
                        )

                        # MR analysis #
                        # Inserts segmentation
                        names_mr, centers_mr, inserts_mr = Auxiliar_functions.inserts_mr(
                            im, coord_mr, final_coord, serie_mr.GetSpacing()
                        )

                        # Go through each insert
                        for i in range(len(inserts_mr)):
                            # Create a reduced analysis region inside the insert for intensity quantification
                            segm_mr, diameter_in_mr = Auxiliar_functions.reduced_segmentation_cyl(
                                inserts_mr[i], float(serie_mr.GetSpacing()[0])
                            )
                    ...
                    for n in range(0, len(names_mr_total)):
                        if len(names_mr_total[n]) != 0 and len(intensity_mr_total[n]) != 0:
                            ...
                            writer = pd.ExcelWriter(
                                os.path.join(output_date_analysis_path, f"Quantification{type_mr_total[n]}.xlsx")
                            )

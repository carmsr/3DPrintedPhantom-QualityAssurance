...
def inserts_ct(im, initial_coord, final_coord, voxeldim, inserts_type):
    ...
    center_1 = np.array(
        (
            central_coord,
            round(center_phantom[0] - ((18 * 1.17188) / float(voxeldim[1]))),
            round(center_phantom[1] + ((83 * 1.17188) / float(voxeldim[0]))),
        )
    )
    center_2 = np.array(
        (
            central_coord,
            round(center_phantom[0] + ((54 * 1.17188) / float(voxeldim[1]))),
            round(center_phantom[1] + ((67 * 1.17188) / float(voxeldim[0]))),
        )
    )
    center_3 = np.array(
        (
            central_coord,
            round(center_phantom[0] + ((86 * 1.17188) / float(voxeldim[1]))),
            round(center_phantom[1] + ((0 * 1.17188) / float(voxeldim[0]))),
        )
    )
    center_4 = np.array(
        (
            central_coord,
            round(center_phantom[0] + ((54 * 1.17188) / float(voxeldim[1]))),
            round(center_phantom[1] - ((66 * 1.17188) / float(voxeldim[0]))),
        )
    )
    center_5 = np.array(
        (
            central_coord,
            round(center_phantom[0] - ((17 * 1.17188) / float(voxeldim[1]))),
            round(center_phantom[1] - ((83 * 1.17188) / float(voxeldim[0]))),
        )
    )
    center_6 = np.array(
        (
            central_coord,
            round(center_phantom[0] - ((75 * 1.17188) / float(voxeldim[1]))),
            round(center_phantom[1] - ((37 * 1.17188) / float(voxeldim[0]))),
        )
    )
    center_7 = np.array(
        (
    ...
    center_8 = np.array(
        (
            central_coord,
            round(center_phantom[0] + ((1 * 1.17188) / float(voxeldim[1]))),
            round(center_phantom[1] + ((34 * 1.17188) / float(voxeldim[0]))),
        )
    )
    center_9 = np.array(
        (
    ...
    center_10 = np.array(
        (central_coord, round(center_phantom[0] - ((33 * 1.17188) / float(voxeldim[1]))), round(center_phantom[1]))
    )
    center_11 = np.array(
        (central_coord, round(center_phantom[0] + ((36 * 1.17188) / float(voxeldim[1]))), round(center_phantom[1]))
    )
    ...
    for center in center_inserts:
        ...
        cyl = generate_cylinder_segmentation(
            image, center[1], center[2], radius_inserts[num], initial_coord, final_coord, voxeldim
        )
        ...
        # sitk.WriteImage(cyl_sitk, "C:\\Users\\20970341E\\Documents\\CT_Insert" + str(num) + ".nrrd")
...
def inserts_pet(im, segm, initial_coord, final_coord, voxeldim):
    ...
    center_3 = np.array(
        (
            central_coord,
            round(center_phantom[0] + ((87 * 1.17188) / float(voxeldim[1]))),
            round(center_phantom[1] + ((2 * 1.17188) / float(voxeldim[0]))),
        )
    )
    center_4 = np.array(
        (
            central_coord,
            round(center_phantom[0] + ((55 * 1.17188) / float(voxeldim[1]))),
            round(center_phantom[1] - ((66 * 1.17188) / float(voxeldim[0]))),
        )
    )
    center_5 = np.array(
        (
            central_coord,
            round(center_phantom[0] - ((16 * 1.17188) / float(voxeldim[1]))),
            round(center_phantom[1] - ((83 * 1.17188) / float(voxeldim[0]))),
        )
    )
    center_6 = np.array(
        (
            central_coord,
            round(center_phantom[0] - ((74 * 1.17188) / float(voxeldim[1]))),
            round(center_phantom[1] - ((38 * 1.17188) / float(voxeldim[0]))),
        )
    )
    center_7 = np.array(
        (
            central_coord,
            round(center_phantom[0] - ((75 * 1.17188) / float(voxeldim[1]))),
            round(center_phantom[1] + ((36 * 1.17188) / float(voxeldim[0]))),
        )
    )
    ...
    for center in center_inserts:
        ...
        # sitk.WriteImage(insert, "C:\\Users\\20970341E\\Documents\\PET_Insert_PETMR" + str(num) + ".nrrd")
        # num += 1
...
def reduced_segmentation_cyl(segm, pixel_x):
    ...
    if not np.any(segmentation == 1):
        ...
        cylinder_mask = (
            (distance_to_center <= radius_cylinder) & (z >= (-height_cylinder / 2)) & (z <= (height_cylinder / 2))
        )
...
def maxvalue(image, seg, percent):
    ...
    max_value_percent = np.nanmean(
        img[img >= (percent / 100) * max_value]
    )  # mean of the voxels with value >= percent*max
...
def resol_pet(im, coord, vertices, voxeldim):
    image = sitk.GetArrayFromImage(im)
    shape = image.shape
    ...
    vertices6 = np.array(vertices[0])
    vertices5 = np.array(vertices[1])
    vertices4 = np.array(vertices[2])
    vertices3 = np.array(vertices[3])
    vertices2 = np.array(vertices[4])
    vertices1 = np.array(vertices[5])
    list_vertices = [vertices1, vertices2, vertices3, vertices4, vertices5, vertices6]
    # list_diameters = [5, 7.5, 9, 11, 12, 15]

    finalSegmentation = np.zeros_like(image)
    ...
    for n in range(len(list_vertices)):
        ...
        # th_h_sitk = sitk.GetImageFromArray(th_holes)
        # th_h_sitk.SetDirection(im.GetDirection())
        # th_h_sitk.SetSpacing(im.GetSpacing())
        # th_h_sitk.SetOrigin(im.GetOrigin())
        # sitk.WriteImage(th_h_sitk, "C:\\Users\\20970341E\\Documents\\PET_" + str(n) + ".nrrd")
        ...
        finalSegmentation = finalSegmentation + th_holes * (n + 1)
        ...
        if num_features != 0:
            ...
            print(f"Min value distance (mm): {min_value_distance}")
            if min_value_distance <= 95:
                ...
                dist_original = math.sqrt(
                    (end_coord[1] - starting_coord[1]) ** 2 + (end_coord[2] - starting_coord[2]) ** 2
                )
                ...
                z_lin, y_lin, x_lin = (
                    np.linspace(starting_coord[0], new_end_coord[0], num=60),
                    np.linspace(starting_coord[1], new_end_coord[1], num=60),
                    np.linspace(starting_coord[2], new_end_coord[2], num=60),
                )

                intensity_profile = [
                    (image[int(round(z_lin[i])), int(round(y_lin[i])), int(round(x_lin[i]))]) for i in range(60)
                ]
                max_intensity = max(intensity_profile)
                normalized_profile = [value / max_intensity for value in intensity_profile]

                normalized_profiles.append(normalized_profile)
                startingCoords.append(starting_coord)
                newEndCoords.append(new_end_coord)

                plotImageValues = image[starting_coord[0], :, :]
                plotThreshHolesValues = th_holes[starting_coord[0], :, :]

                plotImageValuesList.append(plotImageValues)
                plotThreshHolesValuesList.append(plotThreshHolesValues)
                ...
                # ax[1].plot(
                #     [starting_coord[2], new_end_coord[2]],
                #     [starting_coord[1], new_end_coord[1]],
                #     color="red",
                #     linewidth=1,
                # )
    ...
    # return contrast_total, holes_total, section_names
    return (
        contrast_total,
        holes_total,
        rc_holes_total,
...
def resol_ct(im, coord, vertices, voxeldim):
    image = sitk.GetArrayFromImage(im)
    ...
    initial_guesses = [
        ...
        np.std(valid_pixels),
        1,
        np.mean(valid_pixels) + 70,
        np.std(valid_pixels) / 2,
    ]
    params, _ = curve_fit(double_gaussian, bin_centers, hist, p0=initial_guesses)
    _, b1, _, _, b2, c2 = params
    centroid1 = b1
    centroid2 = b2
    ...
    resultValuesForPlt2 = valid_pixels, x_fit, y_fit, centroid1, centroid2, th_cyl
    ...
    finalSegmentation = np.zeros_like(th_holes)
    ...
    for n in range(len(list_vertices)):
        ...
        th_h[~segme] = 0
        # th_holes_sitk = sitk.GetImageFromArray(th_h)
        # th_holes_sitk.SetDirection(im.GetDirection())
        # th_holes_sitk.SetSpacing(im.GetSpacing())
        # th_holes_sitk.SetOrigin(im.GetOrigin())
        # sitk.WriteImage(th_holes_sitk, f"C:\\Users\\20970341E\\Documents\\CT_{n}.nrrd")
        ...
        finalSegmentation = finalSegmentation + th_h * (n + 1)
        ...
        plotImageValues = image[starting_coord[0], :, :]
        plotThreshHolesValues = th_h[starting_coord[0], :, :]
        ...
        # ax[1].plot(
        #     [starting_coord[2], new_end_coord[2]], [starting_coord[1], new_end_coord[1]], color="red", linewidth=1
        # )
        # ax[1].set_title(f"Profile Line")
        #
...
def resol_mr_t2(im, coord, vertices):
    image = sitk.GetArrayFromImage(im)
    shape = image.shape
...
def resol_mr_t1(im, coord, vertices, voxeldim):
    image = sitk.GetArrayFromImage(im)
    shape = image.shape
...
def th_cylinsert(im, in_sl, fin_sl, voxeldim, center_phantom, filled_cyl, modality):
    ...
    if modality == "CT":
        ...
        if filled_cyl:
            ...
            plt.imshow(threshold_str[75, :, :], cmap="gray")
            plt.show()
...
def ct_th_panalinsert(im, panal_slice, center_phantom, voxeldim):
    ...
    new_segm = np.zeros_like(threshold, dtype=float)
...
def calculate_mm_distances(coord_list, y_spacing, x_spacing):
    ...
    for i in range(1, len(coord_list)):
        ...
        # Calcular la distancia euclidiana entre dos puntos adyacentes
...
def resampling(fixed_image, moving_image):
    ...
    moving_resampled = sitk.Resample(
        moving_image, fixed_image, identity, sitk.sitkNearestNeighbor, 0.0, moving_image.GetPixelID()
    )

    return moving_resampled


def itkToSlicerTransform(itk_transform):
...
def register(
    ...
    registration_method.SetGlobalDefaultNumberOfThreads(0)
    ...
    registration_method.SetMetricSamplingPercentage(samplingRate, 2023)  # for Reproducibility
...
def getPercentInactive(raw, mask, thresh=0.6):
    maskA = sitk.GetArrayFromImage(mask)
    rawA = sitk.GetArrayFromImage(raw)

    mask_bool = maskA.astype("uint8")

    ROI = rawA.copy()
    ROI[mask_bool == 0] = 0
    ROI[mask_bool != 0] = rawA[mask_bool != 0]

    maskInactive = np.zeros(np.shape(ROI))
    maskInactive[ROI > (thresh * np.max(ROI) ** 2)] = 1

    perim = bwperim(maskA, 8)
    ...
    sumVolume = np.sum(maskA)
    percentInactive = sumInactive / sumVolume * 100

    return percentInactive


# Eccentricity feature calculation
def getEccentricity(segm):
...
def get_radiomics(image_path, segm_path, modality, bin_width, params_path):
    ...
    radiomics = {
        ...
        "GLCMEnergy": glcmEnergy,
        "GLCMContrast": glcmContrast,
        "GLCMEntropy": glcmEntropy,
        "GLCMHomogeinity": glcmHomogeinity,
        "GLCMCorrelation": glcmCorrelation,
        "GLCMVariance": glcmVariance,
        "GLCMDissimilarity": glcmDissimilarity,
        ...
        "GLSZMSZN": glszmSZN,
        "GLSZMZP": glszmZP,
        "GLSZMLGLZE": glszmLGLZE,
        "GLSZMHGLZE": glszmHGLZE,
        "GLSZMSALGLE": glszmSALGLE,
        "GLSZMSAHGLE": glszmSAHGLE,
        "GLSZMLALGLE": glszmLALGLE,
        "GLSZMLAHGLE": glszmLAHGLE,
        "GLSZMGLV": glszmGLV,
        "GLSZMZV": glszmZV,
        "GLRLMSRE": glrlmSRE,
        "GLRLMLRE": glrlmLRE,
        ...
        "GLRLMLGRE": glrlmLGRE,
        "GLRLMHGRE": glrlmHGRE,
        "GLRLMSRLGLE": glrlmSRLGLE,
        "GLRLMSRHGLE": glrlmSRHGLE,
        "GLRLMLRLGLE": glrlmLRLGLE,
        "GLRLMLRHGLE": glrlmLRHGLE,
        "GLRLMGLV": glrlmGLV,
        "GLRLMRLV": glrlmRLV,
        "NGTDMCoarseness": ngtdmCoarseness,
        "NGTDMContrast": ngtdmContrast,
        "NGTDMBusyness": ngtdmBusyness,
        "NGTDMComplexity": ngtdmComplexity,
        "NGTDMStrength": ngtdmStrength,
    }

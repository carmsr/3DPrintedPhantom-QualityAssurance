...
This extension enables the analysis of different quality parameters for PET, CT and MR images with the 3D-printed phantom: quantification, resolution, co-registration, distortion and radiomics.
"""
...
class QAHybridLogic(ScriptedLoadableModuleLogic):
    ...
    def getSupportedAnalysesMethods(self):
        analyisMethods = {
            ...
            "CO-Registration": self.doRegistration,
    ...
    def addRadiomicSegmentations(self, radiomicSegmentation, modality, parentNodeID):
        segmentationNodeName = "Radiomics" + modality + "Segmentation_" + str(parentNodeID)
    ...
    def getRadiomicsTableNode(self, modality, parentNodeID=None):
        return self.getTableNode("Radiomics" + modality + "_" + str(parentNodeID), parentNodeID)
    ...
    def createQuantificationTableMR(self, modality, parentNodeID, names, meansCT, devsCT):
        ...
        volumeColCt.SetName("Insert")
        meanColCt = vtk.vtkDoubleArray()
        meanColCt.SetName("Mean (ms)")
        devColCt = vtk.vtkDoubleArray()
        devColCt.SetName("Standard Deviation (ms)")

        diameterCT = list(self.insertPositions.values())

        meansCT, devsCT, names, diameterCT = self.sort_lists(meansCT, devsCT, names, diameterCT, descendingOrder=False)
    ...
    def createQuantificationTableCT(self, parentNodeID, names, meansCT, devsCT):
        ...
        meanColCt = vtk.vtkDoubleArray()
        meanColCt.SetName("Mean (HU)")
        devColCt = vtk.vtkDoubleArray()
        devColCt.SetName("Standard Deviation (HU)")
        densityColCt = vtk.vtkStringArray()
        densityColCt.SetName("Density (g/cm3")

        densityValues = []
        densityValuesPresent = False
        for idx, _ in enumerate(meansCT):
            ...
            if idxP1 in self.insertPositions.keys():
                ...
                if insert_name in self.insertDensities:
                    ...
                    densityFound = True
                    densityValuesPresent = True
            if not densityFound:
                densityValues.append(-1.0)

        diameterCT = list(self.insertPositions.values())

        if densityValuesPresent:
            ...
            meansCT, names, densityValues, devsCT, diameterCT = self.sort_lists(
                meansCT, names, densityValues, devsCT, diameterCT, descendingOrder=False
            )
        ...
        for i in range(0, len(names)):
            ...
            devColCt.InsertNextValue(devsCT[i])
            volumeColCt.InsertNextValue(diameterCT[i])
            if densityValues[i] > -1.0:
                densityColCt.InsertNextValue(densityValues[i])
            else:
                densityColCt.InsertNextValue("")
    ...
    def registrationPlot(self, diceCoefficients, DIF_CORTES, modality):
        ...
        if mean_dice >= 0.91:
            if DIF_CORTES > 2:
                ...
                ax1.legend(
                    [
                        f"DSC coefficient: The PET/{modality} system is not properly co-registered",
                ...
                ax1.legend(
                    [
                        f"DSC coefficient: The PET/{modality} system is well co-registered",
                        f"Mean DSC= {round(mean_dice, 2)} \u00B1 {round(std_dice, 2)} and $\Delta$z={round(DIF_CORTES, 2)} mm",
                        f"DSC threshold value: 0.91",
                    ],
                    labelcolor=color,
    ...
    def resolutionPlot(
        ...
        for section in range(0, len(plotImageValuesList)):
            ...
            fig.suptitle(f"Resolution {modality} Section {section+1} (D={list_diameters[section]}mm)")
            ax[0].plot(normalized_profile[section])
            ax[0].set_title("Intensity Profile (From Image Center to Edges)")
            ...
            ax[1].grid(False)
            ax[1].set_title("Profile Line")

            filename = os.path.join(
                self.outputDirectory, f"Resolution{modality}_IntensityProfile_Section{section+1}.png"
            )
            plt.savefig(filename, bbox_inches="tight")
            if self.showOutputPlots:
                pm = qt.QPixmap(filename)
                ...
                imageWidget.setScaledContents(True)
                imageWidget.show()
                self.pltPlots[f"Resolution{modality}_IntensityProfile_Section{section+1}"] = imageWidget
            else:
                plt.close(fig)
    ...
    def quantificationPlotPET(self, rc_ac):
        ...
        filename = os.path.join(self.outputDirectory, "QuantificationPET_RC.png")
        plt.savefig(filename, bbox_inches="tight")
        if self.showOutputPlots:
            pm = qt.QPixmap(filename)
            imageWidget = qt.QLabel()
            ...
            imageWidget.setScaledContents(True)
            imageWidget.show()
            self.pltPlots["QuantificationPET_RC"] = imageWidget
        else:
            plt.close()
    ...
    def quantificationPlotCT(self, intensity_ct):
        ...
        if len(insert_position) > 0:
            ...
            filename = os.path.join(self.outputDirectory, "QuantificationCT_CalibrationCurve.png")
            ...
            if self.showOutputPlots:
                ...
                imageWidget.setScaledContents(True)
                imageWidget.show()
                self.pltPlots["QuantificationCT_CalibrationCurve"] = imageWidget
            else:
                plt.close()
    ...
    def createDistortionTablePanalWalls(
        ...
        distortionTableNode = self.getTableNode(
            "Distortion" + modality + "_PanalWalls_" + str(parentNodeID), parentNodeID
        )
    ...
    def createDistortionTableAxialDistances(self, parentNodeID, modality, axial_distance):
        ...
        distortionTableNode = self.getTableNode(
            "Distortion" + modality + "_AxialDistances_" + str(parentNodeID), parentNodeID
        )
        while distortionTableNode.RemoveRow(0):
            pass
    ...
    def createDistortionTableMeshSides(
        ...
        if len(right_sides) != 0 and len(left_sides) != 0 and len(up_sides) != 0 and len(down_sides) != 0:
            centerDevX = [coord[1] for coord in centers_differences]
            centerDevY = [coord[0] for coord in centers_differences]
            ...
            distortionTableNode = self.getTableNode(
                "Distortion" + modality + "_MeshSides_" + str(parentNodeID), parentNodeID
            )
            while distortionTableNode.RemoveRow(0):
                pass
    ...
    def createResolutionTable(self, parentNodeID, sections, contrastCT, holesCT, rcHoles, modality):
        ...
        holesCtCol.SetName("Holes number")
        rcHolesCtCol = vtk.vtkDoubleArray()
        rcHolesCtCol.SetName("RC holes")
        diameterCol = vtk.vtkDoubleArray()
        diameterCol.SetName("Diameter (mm)")
        sectionDiameterMapping = {"1": 5.0, "2": 7.5, "3": 9.0, "4": 11.0, "5": 12.0, "6": 15.0}
    ...
    def addResolutionResult(self, resolutionResult, volume, modalityName, unit):
        ...
        if resolutionResult[9] is not None:
            self.addSitkVolume(
                resolutionResult[9],
                f"ResolutionSegmentedTriangles{modalityName}",
    ...
    def resolutionCallback(self, resultCT, resultPET, resultT1, resultT2):
        ...
        if resultT2:
            self.addResolutionResult(resultT2, self.t2Volume, "MRT2", "ms")
        if resultT1:
            self.addResolutionResult(resultT1, self.t1Volume, "MRT1", "ms")
    ...
    def addDistortionResultPlotPanalWalls(
        ...
        filename = os.path.join(self.outputDirectory, f"Distortion{modality}_PanalWalls.png")
        ...
        if self.showOutputPlots:
            ...
            imageWidget.setScaledContents(True)
            imageWidget.show()
            self.pltPlots[f"Distortion{modality}_PanalWalls"] = imageWidget
        else:
            plt.close(walls_fig)
    ...
    def addDistortionResultPlotSideValues(
        ...
        filename = os.path.join(self.outputDirectory, f"Distortion{modality}_MeshSides.png")
        plt.savefig(filename, bbox_inches="tight")
        if self.showOutputPlots:
            pm = qt.QPixmap(filename)
            imageWidget = qt.QLabel()
            ...
            imageWidget.setScaledContents(True)
            imageWidget.show()
            self.pltPlots[f"Distortion{modality}_MeshSides"] = imageWidget
        else:
            plt.close()
    ...
    def addDistortionResultPlotCentralDeviation(self, sitkImage, modality, centers_differences, z_slices_filtered):
        ...
        if len(centers_differences) > 0:
            ...
            filename = os.path.join(self.outputDirectory, f"Distortion{modality}_MeshCentres.png")
            ...
            if self.showOutputPlots:
                ...
                imageWidget.setScaledContents(True)
                imageWidget.show()
                self.pltPlots[f"Distortion{modality}_MeshCentres"] = imageWidget
            else:
    ...
    def addDistortionResultPlotAxialDistances(
        ...
        filename = os.path.join(self.outputDirectory, f"Distortion{modality}_AxialDistances.png")
        plt.savefig(filename, bbox_inches="tight")
        if self.showOutputPlots:
            pm = qt.QPixmap(filename)
            imageWidget = qt.QLabel()
            ...
            imageWidget.setScaledContents(True)
            imageWidget.show()
            self.pltPlots[f"Distortion{modality}_AxialDistances"] = imageWidget
        else:
            plt.close()
    ...
    def registrationCallback(self, resultCT, resultT1, resultT2):
        ...
        if resultT1:
            self.registrationPlot(resultT1[0], resultT1[1], "MRT1")
    ...
    def distortionCallBack(self, ctResult, t1Result, t2Result):
        ...
        if t1Result is not None:
            self.addDistortionResults(self.t1Volume, "MRT1", t1Result)

        if t2Result is not None:
            self.addDistortionResults(self.t2Volume, "MRT2", t2Result)
    ...
    def distortionStartEndSliceCylinderDetection(self, currentNode, modality, resourcePath, shrinkImage=False):
        ...
        if modality == "T1" or modality == "T2":
            ...
            nodeName = f"DistortionWallPoints_MR{modality}_Right"
    ...
    def doDistortion(self):
        pointsT1 = self.getDistortionPanalInsertPointPositions("MRT1", self.t1Volume)
        pointsT2 = self.getDistortionPanalInsertPointPositions("MRT2", self.t2Volume)
...
class RadiomicsRoutine(SegmentationRoutine):
    ...
    def run(self):
        ...
        if self.t1Volume is not None:
            radiomicValues["MRT1"], radiomicSegmentations["MRT1"] = self.runRadiomicsAnalysis(
                self.t1Volume, self.t1Segment, "T1"
            )
            self.increaseStatusUpdateFunction(70, "Radiomics")
        if self.t2Volume is not None:
            radiomicValues["MRT2"], radiomicSegmentations["MRT2"] = self.runRadiomicsAnalysis(

# Distortion Analysis for CT and MR DICOM images with the 3D-Printed Phantom.
...
def main_code():
    ...
    for dir in sub_dirs:
        ...
        choices = ['yes', 'no']
        dialog = OptionDialog(window, "Insert filled", f"Has the cylindrical distortion insert been filled for the measurement dated {dir}?", choices)
        ...
        for sub_folder in sub_folders:
            if sub_folder == 'CT':
                in_sl, fin_sl = Auxiliar_functions.select_slices(os.path.join(path_date, sub_folder), sub_folder, 'Choose the initial and final slices of cylindrical insert', dir)
                panal_slice = Auxiliar_functions.select_slices(os.path.join(path_date, sub_folder), sub_folder, 'Choose the distortion panal slice', dir)[0]
            elif sub_folder == 'MR':
                ...
                for mr_image_path in mr_images_path:
                    ...
                    if "T1" in type_mr:
                        in_sl_mr_t1, fin_sl_mr_t1 = Auxiliar_functions.select_slices(mr_image_path, sub_folder + "_" + type_mr, 'Choose the initial and final slices of cylindrical insert', dir)
                        panal_slice_mr_t1 = Auxiliar_functions.select_slices(mr_image_path, sub_folder + "_" + type_mr, 'Choose the distortion panal slice', dir)[0]
                        # Choose panal insert points
                        panal_points_t1 = []
                        directions = ['Right', 'Left', 'Diag. up-right', 'Diag. down-left', 'Diag. up-left', 'Diag. down-right']
                        for i in range(6):
                            coordinat = Auxiliar_functions.choose_points(mr_image_path, panal_slice_mr_t1, sub_folder + "_" + type_mr, directions[i])
                            panal_points_t1.append(coordinat)
                    elif "T2" in type_mr:
                        in_sl_mr_t2, fin_sl_mr_t2 = Auxiliar_functions.select_slices(mr_image_path, sub_folder + "_" + type_mr, 'Choose the initial and final slices of cylindrical insert', dir)
                        panal_slice_mr_t2 = Auxiliar_functions.select_slices(mr_image_path, sub_folder + "_" + type_mr, 'Choose the distortion panal slice', dir)[0]
                        # Choose panal insert points
                        panal_points_t2 = []
                        directions = ['Right', 'Left', 'Diag. up-right', 'Diag. down-left', 'Diag. up-left', 'Diag. down-right']
                        for i in range(6):
                            coordinat = Auxiliar_functions.choose_points(mr_image_path, panal_slice_mr_t2, sub_folder + "_" + type_mr, directions[i])
        ...
        for sub_folder in sub_folders:
            ...
            if sub_folder == 'CT':
                ...
                thresh_cyl_sitk = Auxiliar_functions.th_cylinsert(im, in_sl, fin_sl, ct_spacing, center_phantom, filled_cyl, sub_folder)
                    ...
                    if np.any(z_thresh != 0):
                        ...
                        if len(y_coords) == 0 or len(x_coords) == 0:
                            ...
                            for i in range(n_vertices):
                                ...
                                contiguous_vertices = [vertex for vertex in fused_vertices if np.linalg.norm(vertex - fused_vertex) < 2]
                                if not contiguous_vertices:
                                    fused_vertices.append(fused_vertex)  # Add merged coordinate to the list
                                else:
                                    averaged_vertex = np.mean(np.concatenate((contiguous_vertices, np.array([fused_vertex]))), axis=0)
                                    fused_vertices = [v for v in fused_vertices if not any(np.array_equal(v, vertex) for vertex in contiguous_vertices)]  # Delete contiguous_vertices from fused_vertices
                ...
                plt.legend([f'Distances between axial planes', f'Mean distance in axial axis: {round(mean_axial_distance, 2)} \u00B1 {round(std_value, 2)} mm'])
                plt.ylabel('Measured distance (mm)')
                plt.title(f'Distances along superior-inferior direction of cylindrical insert \n Slice spacing: {round(ct_spacingslices, 2)} mm')
                plt.xticks([])
                ...
                for i in z_slices:
                    ...
                    if len(y_nonzero) >= 7 and len(y_nonzero) <= 9:
                        ...
                        distances = [np.sqrt((coord[0] - y_nonzero_mean) ** 2 + (coord[1] - x_nonzero_mean) ** 2) for coord in combined_nonzero_coords]
                        ...
                        neigh_coords = [coord for coord in combined_nonzero_coords if np.sqrt(((coord[0] - y_central)*ct_spacing[1])**2 + ((coord[1] - x_central)*ct_spacing[0])**2) <= 16]
                ...
                for i, side_list in enumerate([right_sides, left_sides, up_sides, down_sides]):
                    ...
                    plt.axhline(y=mean_value, color=sides_colors[i], label=f'Mean {sides_labels[i]} side: {round(mean_value, 2)} \u00B1 {round(mean_std, 2)} mm')

                ax.set_ylabel('Measured distance (mm)')
                ax.set_xlabel('Slice')
                ax.set_title(f'Distances along transaxial plane of the cylindrical insert \n Pixel dimension: {round(ct_spacing[0], 2)}x{round(ct_spacing[1], 2)} mm\u00B2')
                ax.legend(fontsize=8, bbox_to_anchor=(1.05, 1), loc='upper left')
                plt.ylim([6, 14])
                sides_fig.savefig(os.path.join(output_date_analysis_path, f'DistortionCT_MeshSides.png'), bbox_inches='tight')
                plt.close(sides_fig)
                ...
                ax.scatter(z_slices_filtered, [coord[0] for coord in centers_differences], color=central_colors[0], label=central_labels[0], marker=markers[0])
                ax.scatter(z_slices_filtered, [coord[1] for coord in centers_differences], color=central_colors[1], label=central_labels[1], marker=markers[1])
                ...
                ax.set_xlabel('Slice')
                ax.set_title(f'Mesh centres deviation \n Pixel dimension: {round(ct_spacing[0], 2)}x{round(ct_spacing[1], 2)} mm\u00B2')
                ax.legend(fontsize=8, bbox_to_anchor=(1.05, 1), loc='upper left')
                ax.set_ylim([lower_limit, upper_limit])
                central_fig.savefig(os.path.join(output_date_analysis_path, f'DistortionCT_MeshCentres.png'), bbox_inches='tight')
                ...
                thresh_panal_sitk = Auxiliar_functions.ct_th_panalinsert(im, panal_slice, center_phantom, serie.GetSpacing())
                ...
                distances_coord_r_mm = Auxiliar_functions.calculate_mm_distances(wall_coord_r, ct_spacing[1], ct_spacing[0])

                wall_coord_l = Auxiliar_functions.panal_walls(pixel_left, coord_left)
                distances_coord_l_mm = Auxiliar_functions.calculate_mm_distances(wall_coord_l, ct_spacing[1], ct_spacing[0])

                wall_coord_dsu = Auxiliar_functions.panal_walls(pixel_ds_up, coord_ds_up)
                distances_coord_dsu_mm = Auxiliar_functions.calculate_mm_distances(wall_coord_dsu, ct_spacing[1], ct_spacing[0])

                wall_coord_dsd = Auxiliar_functions.panal_walls(pixel_ds_down, coord_ds_down)
                distances_coord_dsd_mm = Auxiliar_functions.calculate_mm_distances(wall_coord_dsd, ct_spacing[1], ct_spacing[0])

                wall_coord_dmu = Auxiliar_functions.panal_walls(pixel_dm_up, coord_dm_up)
                distances_coord_dmu_mm = Auxiliar_functions.calculate_mm_distances(wall_coord_dmu, ct_spacing[1], ct_spacing[0])

                wall_coord_dmd = Auxiliar_functions.panal_walls(pixel_dm_down, coord_dm_down)
                distances_coord_dmd_mm = Auxiliar_functions.calculate_mm_distances(wall_coord_dmd, ct_spacing[1], ct_spacing[0])
                ...
                # Iterate over each type of side and its respective values
                for i, walls_list in enumerate([distances_coord_r_mm, distances_coord_l_mm, distances_coord_dsu_mm, distances_coord_dsd_mm, distances_coord_dmu_mm, distances_coord_dmd_mm]):
                    ...
                    ax.scatter(range(len(walls_list)), walls_list, color=walls_colors[i], label=walls_labels[i], marker=markers[i])
                    plt.axhline(y=mean_value, color=walls_colors[i], label=f'Mean {walls_labels[i]} side: {round(mean_value, 2)} \u00B1 {round(mean_std, 2)} mm')

                ax.set_ylabel('Measured distance (mm)')
                ax.set_title(f'Distances between panal insert walls \n Pixel dimension: {round(ct_spacing[0], 2)}x{round(ct_spacing[1], 2)} mm\u00B2')
                plt.ylim([23, 30])
                ax.legend(fontsize=8, bbox_to_anchor=(1.05, 1), loc='upper left')
                ax.set_xticks([])
                walls_fig.savefig(os.path.join(output_date_analysis_path, f'DistortionCT_PanalWalls.png'), bbox_inches='tight')
                plt.close(walls_fig)
                ...
                for mr_image_path in mr_images_path:
                    ...
                    # Obtain phantom center in xy plane
                    center_phantom = Auxiliar_functions.phantom_center(ima, panal_slice_mr, mr_spacingslices, sub_folder)

                    # CYLINDRICAL INSERT #
                    # Threshold containing cylindrical insert
                    thresh_cyl_sitk = Auxiliar_functions.th_cylinsert(ima, in_sl_mr, fin_sl_mr, serie_mr.GetSpacing(), center_phantom, filled_cyl, sub_folder + str(type_mr))
                    thresh_cyl = sitk.GetArrayFromImage(thresh_cyl_sitk)
                    ...
                    for i in z_slices:
                        ...
                        if len(y_nonzero) == 9:
                            ...
                            # Choose the neighbours closest to the central point
                            neigh_coords = [coord for coord in combined_nonzero_coords if np.sqrt(((coord[0] - y_central)*spacing[1]) ** 2 + ((coord[1] - x_central)*spacing[0]) ** 2) <= 12]
                    ...
                    ax.legend(fontsize=8, bbox_to_anchor=(1.05, 1), loc='upper left')
                    plt.ylim([6, 14])
                    sides_fig.savefig(os.path.join(output_date_analysis_path, f'Distortion{mr_img}_MeshSides.png'), bbox_inches='tight')
                    plt.close(sides_fig)
                    ...
                    ax.legend(fontsize=8, bbox_to_anchor=(1.05, 1), loc='upper left')
                    ax.set_ylim([lower_limit, upper_limit])
                    central_fig.savefig(os.path.join(output_date_analysis_path, f'Distortion{mr_img}_MeshCentres.png'), bbox_inches='tight')
                    plt.close(central_fig)

                    # Excel document creation
                    writer_sides = pd.ExcelWriter(os.path.join(output_date_analysis_path, f'Distortion{mr_img}_MeshSides.xlsx'))
                    writer_ax = pd.ExcelWriter(os.path.join(output_date_analysis_path, f'Distortion{mr_img}_AxialDistances.xlsx'))
                    ...
                    for panal_points_dir in panal_points:
                        ...
                        for i in range(len(panal_points_dir) - 1):
                            ...
                            distance = math.sqrt(((y2 - y1)*serie_mr.GetSpacing()[1]) ** 2 + ((x2 - x1)*serie_mr.GetSpacing()[0]) ** 2)
                    ...
                    ax.legend(fontsize=8, bbox_to_anchor=(1.05, 1), loc='upper left')
                    ax.set_xticks([])
                    walls_fig.savefig(os.path.join(output_date_analysis_path, f'Distortion{mr_img}_PanalWalls.png'), bbox_inches='tight')
                    plt.close(walls_fig)

                    # Save distances in each direction in an Excel file
                    writer_panal = pd.ExcelWriter(os.path.join(output_date_analysis_path, f'Distortion{mr_img}_PanalWalls.xlsx'))
